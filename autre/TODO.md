# 📝 TODO - Suivi des tâches

## 🔐 Sécurité

- [ ] Configurer Helmet.js sur le back-end
- [ ] Mettre en place HTTPS via Vercel / Nginx
- [ ] Ajouter le rate limiting
- [ ] Vérifier les en-têtes CORS
- [ ] Valider toutes les entrées (sanitization)

## ☁️ Déploiement / DevOps

- [ ] Créer un monorepo avec `apps/` et `libs/`
- [ ] Ajouter un fichier `.env` pour chaque environnement
- [ ] Dockeriser le back-end
- [ ] Déployer le front-end sur Vercel
- [ ] Déployer l'API sur Railway ou Render
- [ ] Configurer CI/CD (GitHub Actions)



https://www.jesse-zhou.com/
genially-test